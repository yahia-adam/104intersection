...
def cone():
    tangent = (math.radians(float(sys.argv[8])))
    xp = float(sys.argv[2])
    yp = float(sys.argv[3])
    zp = float(sys.argv[4])
    xv = float(sys.argv[5])
    ...
    a = math.pow(xv,2) + math.pow(yv,2) - math.pow((zv * math.tan(tangent)), 2)
    b = 2 * xp * xv + 2 * yp * yv - 2 * zp * zv * math.pow(math.tan(tangent), 2)
    c = math.pow(xp,2) + math.pow(yp,2) - math.pow(zp,2) * math.pow(math.tan(tangent),2)
    delta = ((b * b) - (4 * a * c))
    if (a == 0):
        sys.exit(84)
    x = tool.coordinates
    ...
    print("Cone with a {} degree angle".format(sys.argv[8]))
